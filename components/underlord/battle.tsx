...
import { Skull, Zap, Flame, Heart, Sparkles, Undo2, RotateCcw } from "lucide-react"
...
export function BattleScreen({
  ...
  // Reset combo when active unit changes
  useEffect(() => {
  ...
  function handleHexClick(a: Axial) {
    ...
    if (isHealer && target && target.faction === "minion" && target.id !== active.id) {
      ...
      const out = healUnit(next, active.id, target.id)
      next = out.state
      if (out.hit) {
        haptic.select()
          ...
          >
            ...
            >
              ...
              {pixelTiles.map((t) => {
                ...
                const inHeal = highlights.heal.has(k)
                ...
                return (
                  <polygon
                    ...
                    strokeWidth={inMove || inAttack || inHeal ? 1.8 : 0.7}
